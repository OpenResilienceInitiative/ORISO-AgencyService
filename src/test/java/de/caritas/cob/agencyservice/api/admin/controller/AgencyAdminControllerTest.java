...
public class AgencyAdminControllerTest {
  ...
  public void getRoot_Should_returnExpectedRootDTO() throws Exception {
    this.mvc
        ...
        .andExpect(
            jsonPath(
                "$._links.agencies.href", endsWith("/agencyadmin/agencies?page=1&perPage=20{&q,excludeDeleted,tenantId}")));
  }
  ...
  public void searchAgencies_Should_returnOk_When_requiredPaginationParamsAreGiven()
    ...
    Mockito.verify(this.agencyAdminFullResponseDTO, Mockito.times(1))
        .searchAgencies(any(), eq(0), eq(1), any(), eq(de.caritas.cob.agencyservice.api.admin.service.agency.AgencySearchFilter.none()));
  }

  @Test
  public void searchAgencies_Should_passTheTenantFilterThrough_When_aTraegerIsChosen()
      throws Exception {
    this.mvc
        .perform(get(AGENCY_SEARCH_PATH).param(PAGE_PARAM, "1").param(PER_PAGE_PARAM, "10")
            .param("q", "Zebrafink").param("excludeDeleted", "true").param("tenantId", "40"))
        .andExpect(status().isOk());

    Mockito.verify(this.agencyAdminFullResponseDTO, Mockito.times(1))
        .searchAgencies(eq("Zebrafink"), eq(1), eq(10), any(),
            eq(new de.caritas.cob.agencyservice.api.admin.service.agency.AgencySearchFilter(
                true, 40L)));
  }
  ...
  public void searchAgencies_Should_passExcludeDeletedThrough_When_theTypeAheadAsksForIt()
    ...
    Mockito.verify(this.agencyAdminFullResponseDTO, Mockito.times(1))
        .searchAgencies(eq("Schuldner"), eq(1), eq(10), any(), eq(new de.caritas.cob.agencyservice.api.admin.service.agency.AgencySearchFilter(true, null)));
  }
  ...
  public void searchAgencies_Should_bindUpperCasedSortFieldsSentByAdminFrontend()
    ...
    Mockito.verify(this.agencyAdminFullResponseDTO, Mockito.times(8))
        .searchAgencies(any(), eq(1), eq(10), sortCaptor.capture(), eq(de.caritas.cob.agencyservice.api.admin.service.agency.AgencySearchFilter.none()));
